...
TX_HASH_FORMAT = {"entity_hash": "32s", "merkle_component_hash": "32s"}

TX_HASH_LEN = 64

RECEIPT_SOURCE_FORMAT = {"primary_id": "I", "secondary_id": "I"}

RECEIPT_SOURCE_LEN = 8

RECEIPT_FORMAT = {"size": "I", "version": "H", "type": "H"}
...
def deserialize_header(header_data):
    ...
    header["harvester"] = public_key_to_address(
        unhexlify(header["signer_public_key"])
    )
    return header
...
def deserialize_footer(footer_data, header):
    ...
    if header["type"] == b"8043":  # nemesis
        footer = fmt_unpack(
            footer_data[:IMPORTANCE_FOOTER_LEN], IMPORTANCE_FOOTER_FORMAT
        )
    ...
    while i < len(footer_data):
        ...
        tx_header["payload"] = deserialize_tx_payload(
            footer_data[i + TX_H_LEN : i + tx_header["size"]], tx_header["type"]
        )
        tx_data.append(tx_header)

        total_fee += min(
            tx_header["max_fee"], tx_header["size"] * header["fee_multiplier"]
        )
        tx_count += (
            (1 + tx_header["payload"]["embedded_tx_count"])
            if "embedded_tx_count" in tx_header["payload"]
            else 1
        )
        statement_count += 1
...
def deserialize_tx_payload(payload_data, payload_type):
    ...
    if payload_type == b"414c":  # AccountKeyLinkTransaction
        schema = {"linked_public_key": "32s", "link_action": "B"}
    ...
    # Aggregate
        ...
        while i < 8 + payload["payload_size"]:
            e_tx_header = fmt_unpack(
                payload_data[i : i + EMBED_TX_H_LEN], EMBED_TX_H_FORMAT
            )
            e_tx_header["id"] = e_tx_count + 1  # tx ids are 1-based
            e_tx_header["signer_public_key"] = hexlify(
                e_tx_header["signer_public_key"]
            )
            e_tx_header["type"] = hexlify(e_tx_header["type"][::-1])
            e_tx_header["payload"] = deserialize_tx_payload(
                payload_data[i + EMBED_TX_H_LEN : i + e_tx_header["size"]],
                e_tx_header["type"],
            )
    ...
    # Namespace
        ...
        if payload["registration_type"] == 0:
            ...
            raise ValueError(
                f'Unknown registration type for Namespace RegistrationTransaction: {payload["registration_type"]}'
            )
        del payload["identifier"]

    elif payload_type == b"424e":  # AddressAliasTransaction
        schema = {"namespace_id": "Q", "address": "24s", "alias_action": "B"}
        payload = fmt_unpack(payload_data, schema)

    elif payload_type == b"434e":  # MosaicAliasTransaction
        schema = {"namespace_id": "Q", "mosaid_id": "Q", "alias_action": "B"}
        payload = fmt_unpack(payload_data, schema)
    ...
    # Multisignature
        ...
        if payload["address_additions_count"] > 0:
            payload["address_additions"] = struct.unpack(
                "<" + "24s" * payload["address_additions_count"],
        ...
        if payload["address_deletions_count"] > 0:
            payload["address_deletions"] = struct.unpack(
                "<" + "24s" * payload["address_deletions_count"],
    ...
    # Secret Lock
        ...
        payload = fmt_unpack(payload_data, schema)
        payload["recipient_address"] = encode_address(
    ...
    # Account restriction
        ...
        if payload["restriction_additions_count"] > 0:
            payload["restriction_additions"] = struct.unpack(
                "<" + "24s" * payload["restriction_additions_count"],
                payload_data[
                    i : i + payload["restriction_additions_count"] * 24
                ],
            )
        ...
        if payload["restriction_deletions_count"] > 0:
            payload["restriction_deletions"] = struct.unpack(
                "<" + "24s" * payload["restriction_deletions_count"],
                payload_data[
                    i : i + payload["restriction_deletions_count"] * 24
                ],
            )
        else:
        ...
        if payload["restriction_additions_count"] > 0:
            payload["restriction_additions"] = struct.unpack(
                "<" + "Q" * payload["restriction_additions_count"],
                payload_data[
                    i : i + payload["restriction_additions_count"] * 8
                ],
            )
        ...
        if payload["restriction_deletions_count"] > 0:
            payload["restriction_deletions"] = struct.unpack(
                "<" + "Q" * payload["restriction_deletions_count"],
                payload_data[
                    i : i + payload["restriction_deletions_count"] * 8
                ],
            )
        else:
            payload["restriction_deletions"] = []
        ...
        if payload["restriction_additions_count"] > 0:
            payload["restriction_additions"] = struct.unpack(
                "<" + "2s" * payload["restriction_additions_count"],
                payload_data[
                    i : i + payload["restriction_additions_count"] * 2
                ],
            )
        ...
        if payload["restriction_deletions_count"] > 0:
            payload["restriction_deletions"] = struct.unpack(
                "<" + "2s" * payload["restriction_deletions_count"],
                payload_data[
                    i : i + payload["restriction_deletions_count"] * 24
                ],
            )
    ...
    # Transfer
        ...
        for _ in range(payload["mosaics_count"]):
            mosaic = {}
            mosaic["mosaic_id"] = struct.unpack("<Q", payload_data[i : i + 8])[
                0
            ]
            mosaic["amount"] = struct.unpack(
                "<Q", payload_data[i + 8 : i + 16]
            )[0]
...
def deserialize_receipt_payload(receipt_data, receipt_type):
    ...
    # Transaction Statement
        ...
        receipt_source = fmt_unpack(
            receipt_data[:RECEIPT_SOURCE_LEN], RECEIPT_SOURCE_FORMAT
        )
        ...
        for k in range(receipt_count):
            receipt = fmt_unpack(
                receipt_data[i : i + RECEIPT_LEN], RECEIPT_FORMAT
            )
            receipt["payload"] = deserialize_receipt_payload(
                receipt_data[i + RECEIPT_LEN : i + receipt["size"]],
                receipt["type"],
            )
            i += receipt["size"]
        ...
        raise ValueError(
            f"Unknown receipt payload type encountered: {hex(receipt_type)}"
        )
...
def deserialize_transaction_statements(stmt_data, i):
    ...
    for j in range(count[0]):
        receipt_source = fmt_unpack(
            stmt_data[i : i + RECEIPT_SOURCE_LEN], RECEIPT_SOURCE_FORMAT
        )
        i += RECEIPT_SOURCE_LEN
        ...
        for k in range(receipt_count):
            ...
            receipt["payload"] = deserialize_receipt_payload(
                stmt_data[i + RECEIPT_LEN : i + receipt["size"]],
                receipt["type"],
            )
            i += receipt["size"]
...
def deserialize_address_resolution_statements(stmt_data, i):
    ...
    for j in range(count[0]):
        ...
        for k in range(resolution_count):
            address_resolution = fmt_unpack(
                stmt_data[i : i + ADDRESS_RESOLUTION_LEN],
                ADDRESS_RESOLUTION_FORMAT,
            )
...
def deserialize_mosaic_resolution_statements(stmt_data, i):
    ...
    for j in range(count[0]):
        ...
        statement = {"key": key, "resolutions": []}
        for k in range(resolution_count):
            mosaic_resolution = fmt_unpack(
                stmt_data[i : i + MOSAIC_RESOLUTION_LEN],
                MOSAIC_RESOLUTION_FORMAT,
            )
...
def get_statement_paths(statement_extension=".stmt", block_dir="./data"):
    ...
    statement_paths = glob.glob(
        os.path.join(block_dir, "**", "*" + statement_extension), recursive=True
    )
    statement_format_pattern = re.compile("[0-9]{5}" + statement_extension)
    statement_paths = sorted(
        list(
            filter(
                lambda x: statement_format_pattern.match(os.path.basename(x)),
                statement_paths,
            )
        )
    )
...
def deserialize_statements(statement_paths, db_offset_bytes=DB_OFFSET_BYTES):
    ...
    for path in statement_paths_:
        ...
        while i < len(stmt_data):
            ...
            i, transaction_statements = deserialize_transaction_statements(
                stmt_data, i
            )
            (
                i,
                address_resolution_statements,
            ) = deserialize_address_resolution_statements(stmt_data, i)
            (
                i,
                mosaic_resolution_statements,
            ) = deserialize_mosaic_resolution_statements(stmt_data, i)
...
def deserialize_blocks(
    block_paths,
    save_subcache_merkle_roots=True,
    db_offset_bytes=DB_OFFSET_BYTES,
    save_tx_hashes=True,
):
    for path in block_paths:
        ...
        while i < len(blk_data):
            ...
            footer = deserialize_footer(
                blk_data[i + HEADER_LEN : i + header["size"]], header
            )
            i += header["size"]
            block_hash, generation_hash = struct.unpack(
                "<32s32s", blk_data[i : i + 64]
            )
            i += 64
            ...
            if save_tx_hashes:
                ...
                for _ in range(num_tx_hashes):
                    tx_hashes.append(
                        fmt_unpack(
                            blk_data[i : i + TX_HASH_LEN], TX_HASH_FORMAT
                        )
                    )
                    i += TX_HASH_LEN
            else:
            ...
            if save_subcache_merkle_roots:
                merkle_roots = fmt_unpack(
                    blk_data[i : i + root_hash_len], SUBCACHE_MERKLE_ROOT_FORMAT
                )
            i += root_hash_len
...
def get_block_paths(block_dir, block_extension):
    ...
    block_paths = glob.glob(
        os.path.join(block_dir, "**", "*" + block_extension), recursive=True
    )
    block_paths = tqdm(
        sorted(
            list(
                filter(
                    lambda x: block_format_pattern.match(os.path.basename(x)),
                    block_paths,
                )
            )
        )
...
def main(args):
    ...
    with open(args.block_save_path, "wb") as f_blocks:
        for block in blocks:
            ...
            f_blocks.write(msgpack.packb(block, use_bin_type=True))
    ...
    statements = deserialize_statements(
        get_statement_paths(
    ...
    header_df["dateTime"] = pd.to_datetime(
        header_df["timestamp"],
        origin=pd.to_datetime("2021-03-16 00:06:25"),
        unit="ms",
    )
...
def parse_args(argv):
    ...
    parser.add_argument(
        "--block_save_path",
        type=str,
        default="./block_data.msgpack",
        help="path to write the extracted block data to",
    )
    parser.add_argument(
        "--statement_save_path",
        type=str,
        default="./stmt_data.msgpack",
        help="path to write the extracted statement data to",
    )
    parser.add_argument(
        "--state_save_path",
        type=str,
        default="./state_map.msgpack",
        help="path to write the extracted statement data to",
    )
    parser.add_argument(
        "--header_save_path",
        type=str,
        default="./block_header_df.pkl",
        help="path to write the extracted data to",
    )
    parser.add_argument(
        "--block_extension",
        type=str,
    ...
    parser.add_argument(
        "--db_offset_bytes",
        type=int,
        default=DB_OFFSET_BYTES,
        help="padding bytes at start of storage files",
    )
    parser.add_argument(
        "--save_tx_hashes",
        action="store_true",
        help="flag to keep full tx hashes",
    )
    parser.add_argument(
        "--save_subcache_merkle_roots",
        action="store_true",
        help="flag to keep subcache merkle roots",
    )
    parser.add_argument(
