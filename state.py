...
class XYMStateMap:
    ...
    def __init__(self, state_map={}):
        ...
        if len(state_map):
            state_map = {
                k: {
                    ...
                    "delegation_requests": defaultdict(
                        list, v["delegation_requests"]
                    ),
                    "vrf_key_link": defaultdict(list, v["vrf_key_link"]),
                    "node_key_link": defaultdict(list, v["node_key_link"]),
                    "account_key_link": defaultdict(
                        list, v["account_key_link"]
                    ),
        ...
        self.tracked_mosaics = [
            "0x6bed913fa20223f8",
            "0xe74b99ba41f4afee",
        ]  # only care about XYM for now, hardcoded alias
    ...
    def read_msgpack(cls, msgpack_path):
        ...
        if type(msgpack_path) == str:
            ...
            raise TypeError(
                f"Unrecognized type {type(msgpack_path)} for read_msgpack, path str"
            )
            ...
            if (
                ...
                for mosaic in tx["payload"]["mosaics"]:
                    if hex(mosaic["mosaic_id"]) in self.tracked_mosaics:
                        self.state_map[address]["xym_balance"][
                            height
                        ] -= mosaic["amount"]
                        self.state_map[tx["payload"]["recipient_address"]][
                            "xym_balance"
                        ][height] += mosaic["amount"]
    ...
    def insert_block(self, block):
        ...
        self.state_map[header["beneficiary_address"]]["harvested"][
            height
        ] = header["harvester"]
        if header["harvester"] != header["beneficiary_address"]:
            self.state_map[header["harvester"]]["delegated"][height] = header[
                "beneficiary_address"
            ]
    ...
    def insert_rx(self, rx, height):
        ...
        if rx["type"] in [0x124D, 0x134E]:  # rental fee receipts
            if hex(rx["payload"]["mosaic_id"]) in [
                ...
                self.state_map[rx["payload"]["recipient_address"]][
                    "xym_balance"
                ][height] += rx["payload"]["amount"]
    ...
    def get_harvester_graph(
        ...
        node_size_map = {
            k: {"size": len(v), "color": self.node_color}
            for k, v in node_map.items()
            if len(v) >= min_node_size
        }
        delegate_size_map = {
            k: {"size": len(v), "color": self.delegate_color}
            for k, v in delegate_map.items()
            if len(v) >= min_delegate_size
        }
        ...
        for node, delegates in node_map.items():
            if node in node_size_map:
                ...
                graph.add_edges_from(
                    [(node, d, {"weight": d_map[d]}) for d in d_map]
                )
    ...
    def get_harvester_bubbles(
        ...
        node_size_map = {
            k: {"size": len(v), "color": self.node_color, "type": "node"}
            for k, v in node_map.items()
            if len(v) >= min_node_size
        }
        delegate_size_map = {
